...
class PlannerAgent:
    def __init__(self):
        ...
        self.cycle_buffer = 10
        self.cycle_trigger = 4
        self.w_target = 12.5
        self.w_pursuer = 6.5
        self.w_escape = 0.35
        self.w_trap = 2
    ...
    def plan_action(self,
        ...
        self.last_positions.append(cur)
        if len(self.last_positions) > self.cycle_buffer:
            self.last_positions.pop(0)

        if self.last_positions.count(cur) >= self.cycle_trigger:
            best_escape = -1
            ...
            for move in self.get_all_actions():
                ...
                if not self.is_valid(next_pos, world):
                    continue
                escape_routes = self.count_escape_routes(next_pos, world)
                dist_to_pursuer = self.manhattan(next_pos, predicted_pursuer)
                score = escape_routes + dist_to_pursuer
                if score > best_escape:
                    best_escape = score
                    best_move = move
            return best_move
        ...
        for move in self.get_all_actions():
            ...
            if escape_score <= 2 and dist_to_pursuer < 4:
                trap_penalty = (3 - escape_score) * self.w_trap

            score = (
                self.w_target / (dist_to_target + 1) -
                self.w_pursuer / (dist_to_pursuer + 1) +
                self.w_escape * escape_score -
                trap_penalty
            )
