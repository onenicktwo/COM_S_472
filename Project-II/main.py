...
class Task:
    ...
    def step(self):
        tom_a = Tom.plan_action(self.world, self.state[0], self.state[1], self.state[2])
        jerry_a = Jerry.plan_action(self.world, self.state[1], self.state[2], self.state[0])
        spike_a = Spike.plan_action(self.world, self.state[2], self.state[0], self.state[1])
...
if __name__ == "__main__":
    ...
    for id in range(100):
        for running_id in range(5):
            ...
            if (result[1] == 1):
                ties += 1
            if (result[1] == 0 and result[0] == 3):
                losses += 1
                tom_wins += 1
            if (result[1] == 0 and result[2] == 3):
                losses += 1
                spike_wins += 1
